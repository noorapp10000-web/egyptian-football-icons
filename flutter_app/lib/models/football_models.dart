...
class MatchEventModel {
  ...
  factory MatchEventModel.fromJson(Map<String, dynamic> json) => MatchEventModel(
    ...
    minute: _parseMinute(json['minute']),
    addedTime: _parseMinute(json['addedTime']),
...
int? _parseMinute(dynamic raw) {
  if (raw is num && raw.isFinite) {
    return raw.round().clamp(0, 999).toInt();
  }
  if (raw is! String) return null;
  final western = raw
      .replaceAllMapped(
        RegExp('[٠-٩]'),
        (match) => '٠١٢٣٤٥٦٧٨٩'.indexOf(match.group(0)!).toString(),
      )
      .trim();
  final match = RegExp(r'\d{1,3}').firstMatch(western);
  final value = match == null ? null : int.tryParse(match.group(0)!);
  return value?.clamp(0, 999).toInt();
}
