...
import { EventIcon } from "@/components/hub/event-icon";
import { eventLabel, eventTone } from "@/lib/match-events";
import { intervalForMatchDetail } from "@/lib/refresh-policy";
import { formatKickoff } from "@/lib/time";
import type { MatchDetail } from "@/lib/hub-types";
...
type MatchEvent = MatchDetail["events"][number];
type MatchTeam = MatchDetail["homeTeam"];

function eventBelongsTo(event: MatchEvent, team: MatchTeam) {
  if (event.teamId != null && team.id != null) return event.teamId === team.id;
  return (
    Boolean(event.teamName) &&
    event.teamName?.trim().toLocaleLowerCase() === team.name.trim().toLocaleLowerCase()
  );
}
  ...
  return (
    <section className="rounded-2xl border border-border/70 bg-card p-3">
      <div className="mb-2 flex items-center gap-2 border-b border-border/60 pb-2">
        ...
        <span className="rounded-lg bg-primary/10 px-2 py-1 text-[11px] font-black tabular-nums text-primary">
          {events.length}
        </span>
      </div>
      {events.length === 0 ? (
        <p className="py-4 text-center text-xs text-muted-foreground">لا توجد أحداث</p>
      ) : (
        <div className="space-y-2">
          {events.map((event) => (
            <div
              key={`${event.derived ? "d" : "o"}-${event.id}`}
              className="flex items-start justify-between gap-2 rounded-xl border border-border/60 bg-secondary/25 px-2.5 py-2"
            >
              <span className="flex min-w-0 items-start gap-2">
                <Badge
                  variant="outline"
                  className={`shrink-0 gap-1 text-[10px] ${eventTone(event.type)}`}
                >
                ...
                <span className="min-w-0">
                  ...
                  {(event.relatedPlayer || event.text) && (
                    <span className="block text-[11px] leading-relaxed text-muted-foreground">
                      {event.relatedPlayer ? `${event.relatedPlayer} · ` : ""}
                      {event.text ?? ""}
                    </span>
                  )}
                </span>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border/70 bg-card card-sheen p-4">
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
          ...
          {formatKickoff(m.kickoff, m.kickoffText) && (
            <span className="flex items-center gap-1">
              <CalendarDays className="size-3.5" /> {formatKickoff(m.kickoff, m.kickoffText)}
            </span>
          )}
        </div>
      ...
      >
        ...
        <TabsContent value="events" className="mt-4 space-y-2">
          {m.events.length === 0 ? (
            <ErrorNote>لا توجد أحداث مسجلة لهذه المباراة بعد.</ErrorNote>
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
              <EventColumn
                title={m.homeTeam.name}
                team={m.homeTeam}
                events={m.events.filter((event) => eventBelongsTo(event, m.homeTeam))}
              />
              <EventColumn
                title={m.awayTeam.name}
                team={m.awayTeam}
                events={m.events.filter((event) => eventBelongsTo(event, m.awayTeam))}
              />
            </div>
          )}
        </TabsContent>
